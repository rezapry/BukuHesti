...
class TransactionAdapter (private val transactionList: ArrayList<TransactionModel>) : RecyclerView.Adapter<TransactionAdapter.ViewHolder>(){
    ...
    //menampilkan inset data ke menu utama
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val currentTransaction = transactionList[position]
        holder.tvTransactionTitle.text = currentTransaction.title

        if (currentTransaction.type == 1){
            holder.tvTransactionAmount.setTextColor(Color.parseColor("#A9F61515"))
        }else{
            holder.tvTransactionAmount.setTextColor(Color.parseColor("#61C066"))
        }
        ...
        if (currentTransaction.type == 1){ //icon jika insert data pengeluaran
            holder.typeIcon.setImageResource(R.drawable.expense)
        }else{ //icon jika insert data pemasukan
            holder.typeIcon.setImageResource(R.drawable.income)
        }
    }
