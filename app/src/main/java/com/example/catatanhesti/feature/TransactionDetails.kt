...
class TransactionDetails : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        updateData.setOnClickListener {
            openUpdateDialog(
                intent.getStringExtra("judul").toString()
            )
        }
        ...
        initView() //memanggil metode untuk menginisialisasi setiap item ui
        setValuesToViews() //memanggil metode untuk menampilkan nilai pada database
    }

    //fungsi button hapus(tempat sampah)
    private fun deleteData() {
        val deleteData: ImageButton = findViewById(R.id.deleteData)
        val alertBox = AlertDialog.Builder(this)
        deleteData.setOnClickListener {
            alertBox.setTitle("Apakah kamu yakin?")
            alertBox.setMessage("ingin menghapus transaksi ini?")
            alertBox.setPositiveButton("Iya") { _: DialogInterface, _: Int ->
                deleteRecord(
                    intent.getStringExtra("transaksiID").toString()
                )
            }
            alertBox.setNegativeButton("Tidak") { _: DialogInterface, _: Int -> }
            alertBox.show()
        }
    }
    ...
    private fun deleteRecord(transactionID: String) {
        ...
        if (uid != null) {
            ...
            mTask.addOnSuccessListener {
                Toast.makeText(this, "Data Transaksi Terhapus", Toast.LENGTH_LONG).show()
                finish()
            }.addOnFailureListener { error ->
                Toast.makeText(this, "Hapus Error ${error.message}", Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    //fungsi jika data di menu ditampilkan
    private fun setValuesToViews(){

        tvTitleDetails.text =  intent.getStringExtra("judul")
        val type: Int = intent.getIntExtra("tipe",0)
        val amount: Double = intent.getDoubleExtra("jumlah", 0.0)
        tvAmountDetails.text = amount.toString()

        if (type == 1) {
            tvTypeDetails.text = "Transaksi Pengeluaran"
            tvAmountDetails.setTextColor(Color.parseColor("#A9F61515"))
            detailsTitle.setBackgroundResource(R.drawable.bg_detail_expense)
        }else{
            tvTypeDetails.text = "Transaksi Pemasukan"
            tvAmountDetails.setTextColor(Color.parseColor("#61C066"))
            detailsTitle.setBackgroundResource(R.drawable.bg_detail_income)
            window.statusBarColor = ContextCompat.getColor(this, R.color.orangePrimary)
        }

        val date: Long = intent.getLongExtra("tanggal", 0)
        ...
        tvCategoryDetails.text =  intent.getStringExtra("kategori")
        tvNoteDetails.text =  intent.getStringExtra("note")

    }

    //ui dan fungsi saat melakukan update data
    private fun openUpdateDialog(
        ...
        //data judul
        etTitle.setText(intent.getStringExtra("judul").toString())

        //data jumlah
        etAmount.setText(intent.getDoubleExtra("jumlah", 0.0).toString())
        etNote.setText(intent.getStringExtra("note")).toString()

        val type: Int = intent.getIntExtra("tipe",0)
        val transactionID = intent.getStringExtra("transaksiID")

        //data kategori
        val categoryOld = (intent.getStringExtra("kategori"))
        ...
        if (type == 1) { // menu pengeluaran
            etCategory.setAdapter(expenseAdapter)
        }
        if (type == 2){ // Menu pemasukan
        ...
        var dateUpdate: Long = intent.getLongExtra("tanggal", 0)
        ...
        //judul diatas menu update
        mDialog.setTitle("Perbarui Transaksi $title")


        //fungsi tombol perbarui
        ...
        btnUpdateData.setOnClickListener {
            ...
            Toast.makeText(applicationContext, "Data Tansaksi Diperbarui", Toast.LENGTH_LONG).show()
